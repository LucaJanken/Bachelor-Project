...
def onclick(event, par_i, par_j, pi, pj):
    ...
    Z = interp(X, Y)
    c = 'r'
    ax.contour(X,Y,Z,levels=[lkl_min_global+11.83],zorder=4,colors=c,linewidths=3)
    ax.contour(X,Y,Z,levels=[lkl_min_global+6.18],zorder=4,colors=c,linewidths=3)
    ax.contour(X,Y,Z,levels=[lkl_min_global+2.3],zorder=4,colors=c,linewidths=3)
