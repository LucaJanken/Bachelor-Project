...
profile_parser.add_argument("-s", "--batch-size", default=1, type=int, help="Batch size of optimisation")
profile_parser.add_argument("-C", "--chunk-size", default=5, type=int, help="Chunk size for MPI processes")
...
profile_parser.add_argument("-D", "--only-2D", action="store_true", help="Compute only the 2D profiles")
profile_parser.add_argument("-g", "--only-global", action="store_true", help="Compute only the global best-fit")
profile_parser.add_argument("-b", "--1D-bins", default=50, type=int, help="Number of bins for 1D histogram")
profile_parser.add_argument("-B", "--2D-bins", default=20, type=int, help="Number of bins for 2D histogram")
...
if keyword == 'profile':
    ...
    #from source.profile_likelihoods.tf_bao import TF_bao as bao
    #from source.profile_likelihoods.tf_bao_boss import TF_bao_boss as bao
    from source.profile_likelihoods.tf_bao_boss_dr12 import TF_bao_boss_dr12 as bao
    ...
    if config['add_points'] == None and config['recalculate'] == None and not config['fix_points']:
        if calc_1d:
            vals_array_1d, idxs_array_1d, k_array_1d = get_data.get_data_from_mcmc_bins_1d(param_names, param_indices, batch_size, chain_folder, config['1D_bins'])
        if calc_2d:
            vals_array_2d, idxs_array_2d, k_array_2d = get_data.get_data_from_mcmc_bins_2d(param_names, param_indices, batch_size, chain_folder, config['2D_bins'])
    ...
    tf_bao = bao(model_name=connect_model)
    ...
    if bao_bool and planck_bool:
        loglkl_func = lambda x: tf_planck.get_loglkl(x) + tf_bao.loglkl(x)
    elif planck_bool:
        loglkl_func = lambda x: tf_planck.get_loglkl(x)
    elif bao_bool:
        loglkl_func = lambda x: tf_bao.loglkl(x)
    else:
        raise AssertionError('No likelihood given!')
    ...
    class Basinhopping():
        def __init__(self, N_steps, batch_size):
            ...
            data_mins = -loglkl_func(self.params)[:,0].numpy()
            loglike_start = np.min(data_mins)
            self.loc = self.params[np.argmin(data_mins)]
            self.loc = tf.constant([0.022331407, 0.119127244, 67.6395, 3.15, 0.96602577, 0.01, 1.0], dtype=tf.float32)
        ...
        def add_smooth_wall_as_prior_bound(self, parameters, prior_param, prodpri):
            loglike = loglkl_func(parameters) - 100000*(tf.exp(tf.norm(parameters-prior_param, axis=1, keepdims=True)) - 1) * tf.cast(tf.equal(prodpri,0),tf.float32)
            return loglike
    ...
    with MPICommExecutor(comm=comm, root=0) as executor:
        if executor is not None:
            ...
            if calc_1d:
                answers_1d = executor.map(BH.calc_point, zip(vals_array_1d, idxs_array_1d, k_array_1d), unordered=False, chunksize=config['chunk_size'])
            if calc_2d:
                answers_2d = executor.map(BH.calc_point, zip(vals_array_2d, idxs_array_2d, k_array_2d), unordered=False, chunksize=config['chunk_size'])
