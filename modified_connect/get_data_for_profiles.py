...
def get_data_from_mcmc_bins_2d(param_names, param_indices, batch_size, chain_folder, bins=20):
    vals_array = []
    idxs_array = []
    k = 0
    k_array = []
...
def get_data_from_mcmc_bins_1d(param_names, param_indices, batch_size, chain_folder, bins=50):
    vals_array = []
    idxs_array = []
    k = 0
    k_array = []
    ...
    for i, par in enumerate(param_names):
        ...
        hist, xedges = np.histogram(
            chains[:, chain_idx+1], bins=bins,
            weights=chains[:, 0], density=False)
